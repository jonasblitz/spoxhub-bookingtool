...
async function createBike(customerRecordId, bikeData = {}, vehicleType) {
  const { tables } = config();
...
async function createBooking({
  ...
  const fields = {
    ...
    AddressNotes:      state.addressNotes || undefined,
  ...
  // Traffic-Source-Felder (last-touch attribution). Werden vom Frontend
  // (public/js/source.js) ins BookingState geschrieben.
  const s = state.source;
  if (s && s.classification) {
    fields.Source      = s.classification;
    if (s.utmSource)   fields.UtmSource   = String(s.utmSource).slice(0, 255);
    if (s.utmMedium)   fields.UtmMedium   = String(s.utmMedium).slice(0, 255);
    if (s.utmCampaign) fields.UtmCampaign = String(s.utmCampaign).slice(0, 255);
    if (s.utmContent)  fields.UtmContent  = String(s.utmContent).slice(0, 255);
    if (s.utmTerm)     fields.UtmTerm     = String(s.utmTerm).slice(0, 255);
    const clickId = s.gclid || s.fbclid;
    if (clickId)       fields.ClickId     = String(clickId).slice(0, 255);
    if (s.referrer)    fields.Referrer    = String(s.referrer).slice(0, 1000);
  }
  if (customerRecordId) fields.Customer = [customerRecordId];
