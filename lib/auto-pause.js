...
function isAutoPauseApt(apt) {
  return (apt.Notes || '').includes(AUTO_PAUSE_MARKER);
}
...
async function syncAutoPauseForDay(calendarId, date) {
  ...
  if (!planned) {
    ...
    for (const a of autoPauseApts) {
      try { await etermin.deleteAppointment(a.ID || a.IID); }
      catch (e) { console.warn(`[auto-pause] delete failed for apt ${a.ID}:`, e.message); }
    }
