 ...
 * @param {boolean} [opts.allowOverbooking] Wenn true, wird eTermins Kollisions-/
 *   Kapazitätsprüfung umgangen (Termine auch auf belegten Slots). Default false.
 * @param {boolean} [opts.notifyCustomer] Wenn false, verschickt eTermin KEINE
 *   Terminbestätigung an den Kunden (sendemail=0). Default true.
 * @returns {Promise<{ eterminBookingId: (string|number|null), calendarId: number, airtable: object|null }>}
 */
async function createBookingFromState(state, { publicBase = '', calendarId = null, allowOverbooking = false, notifyCustomer = true } = {}) {
  const c = state.customer || {};
  const slot = state.selectedSlot || {};
  ...
  const result = await etermin.createAppointment({
    ...
    sendEmail:          notifyCustomer !== false
  });
