...
async function createAppointment({
  ...
  // Kollisions-/Kapazitätssteuerung — NUR für die externe API relevant:
  //   allowOverbooking === true  → capused=0   (Overbooking erlaubt; Default)
  //   allowOverbooking === false → checkexist=1 (eTermin-Prüfung erzwingen →
  //                                 409 "Time span already blocked" bei Belegung)
  //   undefined (Web-Flow)       → nichts setzen → unverändertes Verhalten
  // Hinweis: eTermin prüft per Default NICHT (ohne checkexist=1), daher ist
  // checkexist=1 nötig, um die Prüfung überhaupt zu aktivieren.
  if (allowOverbooking === true)  params.set('capused', '0');
  else if (allowOverbooking === false) params.set('checkexist', '1');
