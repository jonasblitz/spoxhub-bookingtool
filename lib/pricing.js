...
// Inspektions-Bonus: Fallback wenn Airtable-Konfiguration nicht erreichbar.
// Live-Werte kommen aus lib/config.js (Tabelle Konfiguration).
const INSPEKTION_FREE_MINUTES_FALLBACK = 60;
const RATE_PER_MINUTE_FALLBACK = 2; // €/min — universal
...
async function calculatePricing({ serviceIds, quantities, vehicleType, locationType, travelTimeMinutes }) {
  ...
  // Konfiguration parallel laden (cached) — Fallback auf Code-Defaults bei Fehler.
  const INSPEKTION_FREE_MINUTES = await config.get('InspektionFreeMinutes', INSPEKTION_FREE_MINUTES_FALLBACK);
  const RATE_PER_MINUTE         = await config.get('RatePerMinuteEUR',      RATE_PER_MINUTE_FALLBACK);
  ...
  // Einmaliger Auftrags-Puffer: zwischen Terminen X Min Zeit zum
  // Aufräumen/Übergeben. Wird in der Slot-Engine und im eTermin-Block
  // berücksichtigt; die Sidebar zeigt weiterhin die Service-Dauer ohne
  // Puffer (estimatedDurationMinutes). Wert kommt aus Airtable, Fallback 15.
  const APPOINTMENT_BUFFER_MINUTES = await config.get('AppointmentBufferMinutes', 15);
  const appointmentDurationMinutes = estimatedDurationMinutes + APPOINTMENT_BUFFER_MINUTES;
