...
async function createProfileFromExternalData(authUser) {
  ...
  // Fallback für User ohne Bestandsdaten (typisch für Google-First-Login mit
  // neuer Email): Email-Präfix als firstName. public.customers.first_name
  // ist NOT NULL, daher niemals null einfügen.
  const emailPrefix = String(email.split('@')[0] || 'Kunde').slice(0, 60);
  const first_name = pickFirst(a.Vorname, e.FirstName) || emailPrefix;
